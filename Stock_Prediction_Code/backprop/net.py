...
class network:
    ...
    def weight_initialize(self, net_params):
        self.weight.append(
            net_params.weight_init * np.random.rand(net_params.hidden0, net_params.input_size + 1) * 10 - 5)
    ...
    def errors_update(self, signal_sample, ans):
        ...
        self.errors[len(self.errors) - 1][0] = (signal_sample.output - ans) * ans
        for i in range(len(self.errors) - 1):
            j = len(self.errors) - 2 - i
            for k in range(len(self.errors[j])):
                error = 0
                for l in range(len(self.errors[j + 1])):
                    error += self.nodeactivation[j][k] * (1 - self.nodeactivation[j][k]) * self.errors[j + 1][l] * \
                             self.weight[j][l][k]
