...
def show_one(num):
    ...
    if num == 1:
        length = 100
        f = open('summary_dataset\\back.pkl','rb')
        y1,y2 = pickle.load(f)
        for i in range(len(y1)):
            for j in range(len(y1[0])):
                y1[i][j] = abs(y1[i][j])
    if num == 3:
        f = open('summary_dataset\\e_a.pkl','rb')
        y1,y2,size,batch_num = pickle.load(f)
    if num == 4:
        f = open('summary_dataset\\le_ea.pkl','rb')
        y1,y2,size,batch_num = pickle.load(f)
    if num == 5:
        f = open('summary_dataset\\elitist.pkl','rb')
        y1,y2,size,batch_num = pickle.load(f)
    if num == 6:
        f = open('summary_dataset\\inherit.pkl','rb')
        y1,y2,size,batch_num = pickle.load(f)
    if num == 7:
        f = open('summary_dataset\\dynamic.pkl','rb')
