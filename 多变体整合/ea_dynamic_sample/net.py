...
def produce_offspring(species,params,flag):
    species.sort(key = function.getfitness,reverse=True)
    if flag == True:
        parent_num = params.parent_num/params.process_num/2
    else:
...
def evolve(species,params):
    ...
    for i in range(params.num_gen):
        ...
        new_species = produce_offspring(species,params,flag)
        if i % 50 ==0 :
            print(i)
            ...
            species = function.eval_all(species,sample_list)
            new_species = function.eval_all(new_species,sample_list)
